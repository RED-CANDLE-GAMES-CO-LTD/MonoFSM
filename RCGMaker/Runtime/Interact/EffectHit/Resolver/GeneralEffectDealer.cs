...
namespace RCGMaker.Runtime.Interact.EffectHit
{
    ...
    public class GeneralEffectDealer : EffectResolver, IEffectDealer
    {
        ...
        public IVarMonoProvider proxyProvider;

        [PreviewInInspector]
        GeneralEffectDealer proxyDealer =>
            proxyProvider?.Value?.GetDealer(EffectType); //兩個都可以執行耶，那EffectHitData怎麼算呢？ ex: 人dealer耗體力，斧頭dealer耗耐久
        ...
        public bool CanHitReceiver(IEffectReceiver receiver)
        {
            ...
            if (proxyProvider != null) //指定需要透過ProxyProvider拿 ex: 斧頭上的Dealer
            {
