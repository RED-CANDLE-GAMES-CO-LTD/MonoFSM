...
namespace MonoFSM.Runtime.Attributes
{
    ...
    public class ValueTypeValidateAttribute : Attribute
    {
        ...
        {
            ExpectedType =
                expectedValueType ?? throw new ArgumentNullException(nameof(expectedValueType));
        }
        ...
        public ValueTypeValidateAttribute(string dynamicTypeMethodName)
        {
            DynamicTypePropertyName =
                ...
                ?? throw new ArgumentNullException(nameof(dynamicTypeMethodName));
        }
        ...
        public IEnumerable<Type> GetExpectedTypes(object target)
        {
            ...
            if (ExpectedType != null)
                result.Add(ExpectedType);

            // 多型別陣列模式
            if (ExpectedTypes != null)
                result.AddRange(ExpectedTypes);
            ...
            if (!string.IsNullOrEmpty(DynamicTypePropertyName) && target != null)
            {
                ...
                if (method == null)
                    method = ReflectionHelperMethods.GetNonPublicPropertyInBaseClasses(
                        target.GetType(),
                        DynamicTypePropertyName
                    );
                ...
                if (method != null)
                    try
                    {
                        ...
                        else if (methodResult is Type singleType)
                            result.Add(singleType);
                    }
                    catch (Exception ex)
                    {
                        Debug.LogError(
                            $"執行動態型別方法 '{DynamicTypePropertyName}' 時發生錯誤: {ex.Message}"
                        );
                    }
                else
                    Debug.LogWarning(
                        $"找不到方法 '{DynamicTypePropertyName}' 在型別 {target.GetType().Name} 中"
                    );
            }
        ...
        /// 檢查是否使用多型別模式
        /// </summary>
        public bool IsMultiTypeMode =>
            ExpectedTypes != null || !string.IsNullOrEmpty(DynamicTypePropertyName);
    }
}
