...
- 2026-09-23 新增 `up asset-refs` / `up why-in-build`（Editor 端 `MonoFSM/1_MonoFSM_Core/Editor/PrefabEditing/AssetDeps.cs`）。起因：Mac build 1.17G 要查 NavMesh / 舊橋 fbx / 原始 otf 是誰拉進 build 的，但 `up refs` 只看 prefab / scene 內的節點，吃 asset 路徑只會噴「不是 prefab」，只好手寫 execute-dynamic-code。依賴邊用 `AssetDatabase.GetDependencies(p, false)`（Editor 依賴，editor-only 欄位也算，所以輸出是上界不是 BuildReport）；最後一跳用 SerializedObject 比對 target 及其 sub-asset 的 instanceID（fbx 被引用的通常是裡面的 Mesh），`prefabOverride` 標 override。故意不走離線索引：fbx / otf 不在索引範圍，而且 override 型引用離線表收不到。Addressables 用反射讀，免得 MonoFSM asmdef 硬依賴。踩過的坑：CLI 傳進來的中文路徑 Unicode 正規化跟 GetDependencies 回的不同，字串比對全 miss → target 一律經 guid 繞一圈換成 AssetDatabase 的寫法；檔案剛被刪、Unity 還沒 refresh 時 guid 查得到但引用已變 missing，會誤報「不在 build 裡」→ 磁碟上沒檔就直接擋下來講清楚。`up refs` 收到非 .prefab / .unity 路徑改成指路到這兩個指令。

- 2026-09-24 `up menu "<menu path>"`（C# `MenuEdit.Execute` → `EditorApplication.ExecuteMenuItem`）：讓外部 CLI（meshy 下載完自動包 prefab）跟 agent 按選單走同一條路，不用每次臨時寫 execute-dynamic-code。失敗回 `FAIL:` 開頭、Python 端 exit 1，並用 TypeCache 掃 `[MenuItem]` 列出最接近的 6 個 path（排序：名字含查詢最後一段 > 同父選單 > 編輯距離）。只掃得到專案 / package 的 `[MenuItem]`，Unity 內建選單列不出來但照樣能執行；path 存在卻回 false = validate 擋掉，訊息會分開講。不進 memo（寫入類）。剛 domain reload 完第一次呼叫可能 timeout，重跑即可。
- `prefab do` / `scene do` inline ops：同一批有 `mark` 卻沒剩任何 `$`、又有參數以 `/` 開頭時，打 Unity 前就擋下來並提示改單引號或 `-f`。雙引號包的 `$SF/...` 會被 zsh 展開成空字串，以前要到 Unity 解節點才失敗、錯誤看不出原因（寶箱 FSM 改動時踩到）。
- 2026-09-25 `find` 節點行帶 `layer=<名字>`（nodes.layer 本來就有存，只是沒印；名字對照 `ProjectSettings/TagManager.asset`，Default 不印）。只反映檔案自己寫的 m_Layer，variant / nested 的 layer override 不在離線索引裡 —— 真值走 Unity 端的 read。
- 2026-09-25 `up prefab bounds <prefab> [--node] [--segments N]`（C# `EditBounds.Bounds`）：active renderer 的 AABB（prefab root 座標系 + `--node` local）加沿最長軸切段的最大半徑，拿來對齊 placeholder 大小、判斷模型哪頭粗（手電筒燈頭）。**故意走 Unity 端、不離線解析 FBX**：起因是 agent 手寫 Python 讀 binary FBX 頂點判斷燈頭，但 importer 的軸向轉換（右手→左手 X 反轉、Z-up、file scale / useFileScale、bakeAxisConversion）只有匯入後的 mesh 才是真值，離線只能「照 Unity 慣例推」、證明不了方向；而且 Unity 會照 UV / normal 拆頂點（FBX 2203 個控制點 → mesh 3985 個），離線數字跟 Editor 對不上。Editor 下 `Mesh.GetVertices` 不受 Read/Write 設定限制，LoadPrefabContents 讀完就 Unload，不 dirty。「active」只看 renderer 到 `--node` 之間的 activeSelf，`--node` 上層關著不算，不然 View 被上層關掉時會整個算不到。
