...
namespace RCGMaker
{
    ...
    public class PlayerLoopWindow : EditorWindow
    {
        ...
        private static PlayerLoopSystem currentPlayerLoop = new();

        private static PlayerLoopSystem
            nextPlayerLoop; // used for storing changes to the loop. Applied at the end of the GUI draw.
        ...
        private PlayerLoopSystem GenerateCustomLoop()
        {
            ...
            var beginUpdateSystem = new PlayerLoopSystem();
            beginUpdateSystem.type =
                typeof(CustomPlayerLoopStartUpdate); // Unity uses the name of the type here to identify the System - we would see this show up in the Profiler for example
            beginUpdateSystem.updateDelegate =
                CustomPlayerLoopStartUpdate
                    .UpdateFunction; // we can plug a C# method into this delegate to control what actually happens when this System updates
            ...
            newList.Add(CustomPlayerLoopEndUpdate
                .GetNewSystem()); // this time, lets use a small static helper method i added to the Systems type to generate the PlayerLoopSystem. this pattern is much cleaner :)
        ...
        private Stack<PlayerLoopSystem> systemStack = new();

        private void DrawSubsystemList(PlayerLoopSystem system, int increment = 1)
        {
            ...
            using (new EditorGUI.IndentLevelScope(increment))
            {
                ...
                var header = system.subSystemList != null;
                if (header)
                {
                    ...
                    var fold = EditorGUILayout.Foldout(GetFoldout(fullName), name,
                        true); // use the GetFoldout helper method to see if its open or closed
                ...
                else
                {
                    ...
                    var custom = system.updateDelegate != null;
                    using (new EditorGUI.DisabledScope(custom))
                    {
                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Space((float)EditorGUI.indentLevel *
                                        18f); // indent the entry nicley. We have to do this manually cos the flexible space at the end conflicts with GUI.Indent
        ...
        private void RemoveSystem(PlayerLoopSystem target, PlayerLoopSystem playerLoopSystem)
        {
            ...
            for (var i = 0; i < playerLoopSystem.subSystemList.Length; i++)
            {
                ...
                if (system.type == target.type)
                {
                    ...
                    for (var j = 0; j < currentPlayerLoop.subSystemList.Length; j++)
                    {
