...
//FIXME: 還是要可以帶一個變數會比較好 (或是一組變數？可以 remapping的？) 畢竟就算要 add force 之類的還是有可能會有多種力道之類的
    public class GeneralEffectDealer : EffectResolver, IEffectDealer
    {
        ...
        //_conditions.IsAllValid()，完全不看 culling。dealer 被 culling handle 關掉時
        //isActiveAndEnabled 是 false，直接早退的話這顆 getter 會讀到「沒打到」——
        ...
        //NotQueried 是 0，不寫 initializer（hot reload 對 added field 的 initializer 有限制）
        private OverlapQueryState _overlapQueryState;

        [Conditional("UNITY_EDITOR")]
        private void SetOverlapQueryState(OverlapQueryState state)
        {
            _overlapQueryState = state;
        }
